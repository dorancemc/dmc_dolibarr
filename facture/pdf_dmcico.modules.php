 ...
 * Diego Cruz       <ing.diego.cruz@hotmail.com> 
 * To compliance on Colombia's invoice models
 */

/**
 *	\file       htdocs/core/modules/facture/doc/pdf_dmcico.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from dmcico model
 */
 ...
 */
class pdf_dmcico extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
...
//		}
		...
		if ($showaddress)
		{
			...
			$pdf->SetFont('','B', $default_font_size);
			$pdf->MultiCell(80, 4, $outputlangs->convToOutputCharset($this->emetteur->name)."\n".$outputlangs->transnoentities("VATIntra")." ".$this->emetteur->tva_intra, 0, 'L');
	...
	function _pagefoot(&$pdf,$object,$outputlangs,$hidefreetext=0)
	{
		...
		$marginwithfooter=$this->marge_basse + $freetextheight + (! empty($line1)?3:0) + (! empty($line2)?3:0) + (! empty($line3)?3:0) + (! empty($line4)?3:0);
		...
		//$marginwithfooter=$this->marge_basse;
		//$posy=$marginwithfooter+0;
				
		$pdf->SetY(-$posy);
		$pdf->line($dims['lm'], $dims['hk']-$posy, $dims['wk']-$dims['rm'], $dims['hk']-$posy);
				
		$line1 = $this->emetteur->name." ".$outputlangs->transnoentities("VATIntra").": ".$this->emetteur->tva_intra." - ".$this->emetteur->idprof6;
